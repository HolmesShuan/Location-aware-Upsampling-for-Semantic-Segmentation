...
from encoding.nn import SegmentationLosses, BatchNorm2d, OffsetLosses
...
class Trainer():
    def __init__(self, args):
        ...
        for key, value in param_dict.items():
            if 'diffUp' in key:
                # offsset
                params_list.append({'params':[value], 'weight_decay': 0.0, 'lr': args.lr*10})
                print('predict offset conv : ', key)
        ...
        self.criterion = OffsetLosses(se_loss=args.se_loss, aux=args.aux,
                                            nclass=self.nclass, 
                                            se_weight=args.se_weight,
                                            aux_weight=args.aux_weight,
                                            offset=args.offset_loss, 
                                            offset_weight=args.offset_weight,
                                            location_regression_weight=args.location_weight)
    ...
    def training(self, epoch):
        ...
        if self.args.no_val:
            ...
            if epoch >= args.epochs-8:
                ...
                }, self.args, is_best, str(epoch)+'_checkpoint.pth.tar')
            else:
                utils.save_checkpoint({
                    'epoch': epoch + 1,
                    'state_dict': self.model.module.state_dict(),
                    'optimizer': self.optimizer.state_dict(),
                    'best_pred': self.best_pred,
                }, self.args, is_best, 'tmp_checkpoint.pth.tar')
