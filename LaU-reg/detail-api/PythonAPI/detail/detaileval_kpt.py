__author__ = 'zengxh'
__version__ = '4.0'
# Interface for evaluating the PASCAL in Detail dataset(keypoints). 
...
class DetailEvalKpt:
    def __init__(self, detailGt=None, iouType='keypoints'):
        '''
        ...
        self.cats = [284] # person category only
        self.num_cats = 1 #len(self.cats)
        self.num_cats = len(self.cats)
        self.gt_imgs = self.detail_gt.getImgs(cats=['person'])
    ...
    def createIndex(self): 
        ...
        for i in range(self.num_img):
            ...
            self.params.imgIds.append(self.gt_imgs[i]['image_id'])
        self.cat_idxs = {} #np.zeros([self.num_img])
 
    def loadRes(self, detailRes):
        self.results = detailRes
    ...
    def _prepare(self):
        """
        Prepare self._gts and self._dts for Evaluation
        for each image, 
            self._gts[imgId, 284]= [
                {'keypoints': ..., 'instance_id': ..., 'bbox': ..., 'area: ...'}, 
        ...
        for count, dt in enumerate(self.results):
            ...
            dt['area'] = (x1-x0)*(y1-y0)
            dt['id'] = count + 1
            dt['bbox'] = [x0,y0,x1-x0,y1-y0]
            self._dts[dt['image_id'], category_id].append(dt)
    ...
    def computeOks(self, imgId, catId=284):
        p = self.params
        ...
        sigmas = np.ones(14)*0.5 #np.array([.26, .25, .25, .35, .35, .79, .79, .72, .72, .62,.62, 1.07, 1.07, .87, .87, .89, .89])/10.0
        vars = (sigmas * 2)**2
        k = len(sigmas)
...
class Params:
    ...
    def setKpParams(self):
        ...
        self.recThrs = np.linspace(.0, 1.00, np.round((1.00 - .0) / .01) + 1, endpoint=True)
        self.maxDets = [20]
        #self.areaRng = [[0 ** 2, 1e5 ** 2]]     # use all area in PASCAL Detail
        #self.areaRngLbl = ['all']
