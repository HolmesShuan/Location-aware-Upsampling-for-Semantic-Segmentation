...
class bboxEval:
    def __init__(self,details):
        self.params = param.Params(iouType='bbox')
    ...
    def loadBoxRes(self, resFile='/Users/zhishuaizhang/github_repo/detail-api/res/instances_val2014_fakebbox100_results.json'): # Zhishuai
        '''
        ...
        if self.params.useCat:
            self.dts = [(_['image_id'],_['category_id'],_['bbox'],_['score']) for _ in anns]
        else:
            self.dts = [(_['image_id'],-1,_['bbox'],_['score']) for _ in anns]
    ...
    def evaluate(self):
        ...
        :return: None
        '''
        self.gts = [(i,self.details.find_cat(j['category']),j['bbox']) for i in self.params.imgIds for j in self.details.getBboxes(i)]
        imgIds_ = self.params.imgIds
        #self._prepare()
        catIds_ = self.params.catIds if self.params.useCat else [-1]

        maxDet_ = self.params.maxDets[-1]

        self.ious = {(imgId, catId): self._computeIoU(imgId, catId, self.params.useCat) \
    ...
    def _computeIoU(self, imgId, catId, useCat = True):
        maxDet_ = self.params.maxDets[-1]
        if useCat:
            gt = [_ for _ in self.gts if _[0] == imgId and _[1] == catId]
            dt = [_ for _ in self.dts if _[0] == imgId and _[1] == catId]
        ...
        if len(dt) > maxDet_:
            dt=dt[0:maxDet_]
        ious = np.zeros((len(gt), len(dt)))
        for i in range(len(gt)):
            for j in range(len(dt)):
                _inter = (np.minimum(gt[i][2][2]+gt[i][2][0],dt[j][2][2]+dt[j][2][0]) - np.maximum(gt[i][2][0],dt[j][2][0])) *\
                    (np.minimum(gt[i][2][3]+gt[i][2][1],dt[j][2][3]+dt[j][2][1]) - np.maximum(gt[i][2][1],dt[j][2][1]))
                _inter = np.maximum(_inter, 0.)
                _union = gt[i][2][2] * gt[i][2][3] +\
                    dt[j][2][2] * dt[j][2][3] - _inter
                ious[i,j] = _inter / _union
        return ious
    ...
    def accumulate(self, p = None):
        ...
        if p is None:
            p = self.params
        p.catIds = p.catIds if p.useCat == 1 else [-1]
        T           = len(p.iouThrs)
        R           = len(p.recThrs)
        K           = len(p.catIds) if p.useCat else 1
        M           = len(p.maxDets)
        ...
        self.recall = self.eval['recall'][:,0,:]
